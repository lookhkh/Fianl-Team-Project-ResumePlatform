 ...
 <div class="wrapper">
        <div class="introduce" style="text-align: center;">
            <form action="/corporate/register" method="POST">
                <h3>${userName}</h3><br>
                <c:if test="${error.hasFieldErrors('summary')}"><p style="color : red">${error.getFieldError('summary')}</p></c:if>
                <textarea  id="" cols="80" rows="10" style="resize: none;" maxlength="500" name="summary" placeholder="최대 500자 까지 입력가능." value="${detail.summary}">${detail.summary}</textarea>
                <br>
                <label for="hireNum">채용 인원 </label>
                                <c:if test="${error.hasFieldErrors('staffNumber')}"><p style="color : red">${error.getFieldError('staffNumber')}</p></c:if>
                    ...
                    <label for="Salary">종업원 평균 연봉</label><br>
                                                    <c:if test="${error.hasFieldErrors('salary')}"><p style="color : red">${error.getFieldError('salary')}</p></c:if>
